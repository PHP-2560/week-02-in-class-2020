...
* This command updates the column names to year, month, day, and then 0-23 represents the 24 hours in the day. 
...
* There are negative values in our dataset, which doesn't make sense since we are measuring daily rainful. These negative values are gives us negative sum values, which is creating a problem in our dataset and our histogram. 
...
```{r}
library(dplyr)
rain.df.fixed <- mutate_all(rain.df, funs(replace(., .< 0, NA)))
hist(rain.df.fixed$daily)
```

* This histogram is more reasonable because now all the daily rainfall values are positive. 
...
```{r}
#Filtering all flights on March 10
...
```{r}
#Filtering all flights in rows 567-589
filter(flights[567:589, ])
```
...
```{r}
#Arrange by descending arrival delay
...
```{r}
library(plyr)
rename(flights, c("tailnum" = "tail_num"))
```
...
```{r}
#gain describes difference between arrival delay and departure delay
